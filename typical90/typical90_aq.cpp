...
int main()
{
    ...
    map<char, vector<vector<ll>>> T;
    vector<char> Y = {'R', 'D', 'L', 'U'};
    VVI(ll, X, h, w, INFLL);
    T['R'] = X;
    T['D'] = X;
    T['L'] = X;
    T['U'] = X;
    map<char, pair<ll, ll>> V;
    V['R'] = pair(0, 1);
    V['D'] = pair(1, 0);
    V['L'] = pair(0, -1);
    V['U'] = pair(-1, 0);
    ...
    while (Q.size() != 0)
    {
        auto q = Q.front();
        char v = q.first;
        ll y = q.second.first, x = q.second.second;
        Q.pop_front();
        for (char r : Y)
        {
            ll dy = V[r].first, dx = V[r].second;
            if (0 <= y + dy && y + dy < h && 0 <= x + dx && x + dx < w && S[y + dy][x + dx] == '.')
            {
                if (r == v)
                {
                    if (T[r][y + dy][x + dx] > T[v][y][x])
                    {
                        T[r][y + dy][x + dx] = T[v][y][x];
                        if (x + dx != gx || y + dy != gy)
                        {
                            Q.push_back(pair(r, pair(y + dy, x + dx)));
                        }
                    }
                }
                else
                {
                    if (T[r][y + dy][x + dx] > T[v][y][x] + 1)
                    {
                        T[r][y + dy][x + dx] = T[v][y][x] + 1;
                        if (x + dx != gx || y + dy != gy)
                        {
                            Q.push_back(pair(r, pair(y + dy, x + dx)));
                        }
