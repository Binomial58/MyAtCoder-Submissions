...
int main()
{
    ...
    {
        ll t = m - i * i;
        // print((ll)floor(sqrt(t)) * (ll)floor(sqrt(t)), t);
        if ((ll)floor(sqrt(t)) * (ll)floor(sqrt(t)) == t)
        {
            ll j = (ll)floor(sqrt(t));
    ...
    while (Q.size() != 0)
    {
        ...
        for (auto v : V)
        {
            ll dy = v.first, dx = v.second;
            if (0 <= x + dx && x + dx < n && 0 <= y + dy && y + dy < n)
            {
                if (A[y][x] + 1 < A[dy + y][dx + x])
                {
                    A[dy + y][dx + x] = A[y][x] + 1;
                    Q.push_back(pair(dy + y, dx + x));
                }
            }
        }
        // print(Q);
