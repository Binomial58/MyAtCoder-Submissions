...
int main()
{
    // ここにコードを書く
    LL(n, m);
    VECI(ll, L_min, n, INFLL);
    VECI(ll, R_max, n, -INFLL);
    VEC0(vector<ll>, L, n);
    VEC0(vector<ll>, R, n);
    ...
    rep0(i, m)
    {
        ...
        r--;
        mp[pair(l, r)]++;
        L_min[l] = min(L_min[l], r);
        R_max[r] = max(R_max[r], l);
        L[l].push_back(r);
        R[r].push_back(l);
    }
    rrep(i, n - 1, 0)
    {
        L_min[i - 1] = min(L_min[i], L_min[i - 1]);
    }
    rep(i, 1, n - 1)
    {
    ...
    rep0(i, q)
    {
        ...
        else
        {
            ...
            {
                ll li = bisect_right(L[s], t) - 1, ri = bisect_left(R[t], s);
                if (li >= 0 && ri < R[t].size())
                {
                    ll l = L[s][li], r = R[t][ri];
                    // print(L[s]);
                    // print(R[t]);
                    // print("-----------");
                    // print(s, l);
