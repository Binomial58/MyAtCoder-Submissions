...
int main()
{
    ...
    rep(i, 1, 4)
    {
        rep(j, 1, s.size())
        {
            if (s[j] == 'a')
            {
                dp[1][j] = (dp[0][j - 1] + dp[1][j - 1] + dp[2][j - 1] + dp[3][j - 1]) % MOD;
                dp[2][j] = dp[2][j - 1];
                dp[3][j] = dp[3][j - 1];
            }
            else if (s[j] == 'b')
            {
                dp[2][j] = (dp[0][j - 1] + dp[1][j - 1] + dp[2][j - 1] + dp[3][j - 1]) % MOD;
                dp[1][j] = dp[1][j - 1];
                dp[3][j] = dp[3][j - 1];
            }
            else
            {
                dp[3][j] = (dp[0][j - 1] + dp[1][j - 1] + dp[2][j - 1] + dp[3][j - 1]) % MOD;
                dp[1][j] = dp[1][j - 1];
                dp[2][j] = dp[2][j - 1];
            }
        }
    }
    ll ans = dp[1][s.size() - 1] + dp[2][s.size() - 1] + dp[3][s.size() - 1];
    print(ans % MOD);
}
