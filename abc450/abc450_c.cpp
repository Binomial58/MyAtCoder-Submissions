...
int main()
{
    ...
    deque<pair<ll, ll>> Q;
    vector<pair<ll, ll>> V = {pair(0, 1), pair(0, -1), pair(1, 0), pair(-1, 0)};
    // 一番上と下
    rep0(i, w)
    {
        if (S[0][i] == '.')
        {
            Q.push_front(pair(0, i));
            S[0][i] = '#';
            while (Q.size() != 0)
            {
                pair p = Q.front();
                ...
                for (auto v : V)
                {
                    ll dy = y + v.first, dx = x + v.second;
                    if (0 <= dy && dy <= h - 1 && 0 <= dx && dx <= w - 1)
                    {
                        if (S[dy][dx] == '.')
                        {
                            S[dy][dx] = '#';
                            Q.push_front(pair(dy, dx));
                        }
                        // print(y, x);
                        // print(dy, dx);
                    }
