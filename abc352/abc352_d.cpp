...
int main()
{
    ...
    sort(V.begin(), V.end(), [](const vector<ll> &alpha, const vector<ll> &beta)
    ...
    rep0(i, n)
    {
        while (size > 0 && maxD[0] <= i - k)
        {
            maxD.pop_front();
            size--;
        }
        while (size > 0 && V[maxD[size - 1]][1] < V[i][1])
        {
            maxD.pop_back();
            size--;
        }
    ...
    rep0(i, n)
    {
        while (size > 0 && minD[0] <= i - k)
        {
            minD.pop_front();
            size--;
        }
        while (size > 0 && V[minD[size - 1]][1] > V[i][1])
        {
            minD.pop_back();
            size--;
        }
