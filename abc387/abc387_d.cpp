...
int main()
{
    ...
    while (Q.size() != 0)
    {
        // print(Q);
        // print("--------------");
        ll y = Q.front().first.first, x = Q.front().first.second, d = Q.front().second;
        Q.pop_front();
        ll t;
        if (d == 0)
        {
    ...
    if (min(min(R[gy][gx], D[gy][gx]), min(L[gy][gx], U[gy][gx])) == INFLL)
    {
        print(-1);
    }
    else
    {
        print(min(min(R[gy][gx], D[gy][gx]), min(L[gy][gx], U[gy][gx])));
    }
    // print(R);
    // print("---------------");
    // print(D);
